...
ALTER TYPE intent_status ADD VALUE IF NOT EXISTS 'DISPATCHED_SPOKE';
ALTER TYPE intent_status ADD VALUE IF NOT EXISTS 'DISPATCHED_HUB';
ALTER TYPE intent_status ADD VALUE IF NOT EXISTS 'SETTLED_AND_COMPLETED';

DROP MATERIALIZED VIEW IF EXISTS public.intents;

DROP FUNCTION IF EXISTS genStatus(intent_status, intent_status, intent_status);
...
DROP FUNCTION IF EXISTS genStatus(intent_status, intent_status, intent_status, boolean);

CREATE OR REPLACE FUNCTION genStatus(origin_status intent_status, hub_status intent_status, settlement_status intent_status)
RETURNS intent_status AS $$
BEGIN
    IF origin_status = 'DISPATCHED' AND hub_status = 'NONE' THEN
        RETURN 'DISPATCHED_SPOKE';
    ELSIF hub_status = 'NONE' THEN
        RETURN origin_status;
    ELSIF hub_status = 'DISPATCHED' AND settlement_status = 'NONE' THEN
        RETURN 'DISPATCHED_HUB';
    ELSIF hub_status = 'DISPATCHED' THEN
...
CREATE MATERIALIZED VIEW public.intents AS
  SELECT origin_intents.id,
    ...
    hub_intents.settlement_epoch AS hub_settlement_epoch,
    hub_intents.update_virtual_balance AS hub_update_virtual_balance,
    genStatus(origin_intents.status, hub_intents.status, settlement_intents.status) AS status,
    hasCallData(origin_intents.data) AS has_calldata,
    hub_intents.auto_id AS hub_auto_id
  FROM (((public.origin_intents
    LEFT JOIN public.destination_intents ON ((origin_intents.id = destination_intents.id)))
...
ALTER TYPE intent_status DROP VALUE IF EXISTS 'DISPATCHED_SPOKE';
ALTER TYPE intent_status DROP VALUE IF EXISTS 'DISPATCHED_HUB';
ALTER TYPE intent_status DROP VALUE IF EXISTS 'SETTLED_AND_COMPLETED';