...
-- This migration ensures that the order_id field from origin_intents is properly
-- included in the intents and invoices materialized views.
-- This addresses the issue where Chainlink batch creation requires order_id in the API response.

-- Drop and recreate the materialized views to ensure they include order_id
...
-- Recreate intents view with order_id included
CREATE MATERIALIZED VIEW public.intents AS SELECT * FROM (
  SELECT origin_intents.id, origin_intents.queue_idx AS origin_queue_idx, origin_intents.message_id AS origin_message_id,
  origin_intents.status AS origin_status, origin_intents.initiator AS origin_initiator, origin_intents.receiver AS origin_receiver,
  origin_intents.input_asset AS origin_input_asset, origin_intents.output_asset AS origin_output_asset, origin_intents.amount AS origin_amount,
  origin_intents.max_fee AS origin_max_fee, origin_intents.origin AS origin_origin, origin_intents.destinations AS origin_destinations,
  origin_intents.ttl AS origin_ttl, origin_intents.nonce AS origin_nonce, origin_intents.data AS origin_data,
  origin_intents.transaction_hash AS origin_transaction_hash, origin_intents.timestamp AS origin_timestamp,
  origin_intents.block_number AS origin_block_number, origin_intents.gas_limit AS origin_gas_limit, origin_intents.gas_price AS origin_gas_price,
  origin_intents.tx_origin AS origin_tx_origin, origin_intents.tx_nonce AS origin_tx_nonce, origin_intents.auto_id AS origin_auto_id,
  origin_intents.native_fee AS origin_native_fee, origin_intents.token_fee AS origin_token_fee,
  origin_intents.fee_adapter_initiator AS origin_fee_adapter_initiator, origin_intents.order_id AS origin_order_id,
  origin_intents.amount_out_min AS origin_amount_out_min, origin_intents.is_swap AS origin_is_swap,
  destination_intents.queue_idx AS destination_queue_idx, destination_intents.message_id AS destination_message_id,
  destination_intents.status AS destination_status, destination_intents.initiator AS destination_initiator,
  destination_intents.receiver AS destination_receiver, destination_intents.solver AS destination_solver,
  destination_intents.input_asset AS destination_input_asset, destination_intents.output_asset AS destination_output_asset,
  destination_intents.amount AS destination_amount, destination_intents.fee AS destination_fee,
  destination_intents.origin AS destination_origin, destination_intents.destinations AS destination_destinations,
  destination_intents.ttl AS destination_ttl, destination_intents.filled_domain AS destination_filled,
  destination_intents.nonce AS destination_nonce, destination_intents.data AS destination_data,
  destination_intents.transaction_hash AS destination_transaction_hash, destination_intents.timestamp AS destination_timestamp,
  destination_intents.block_number AS destination_block_number, destination_intents.gas_limit AS destination_gas_limit,
  destination_intents.gas_price AS destination_gas_price, destination_intents.tx_origin AS destination_tx_origin,
  destination_intents.tx_nonce AS destination_tx_nonce, destination_intents.auto_id AS destination_auto_id,
  destination_intents.amount_out_min AS settlement_amount_out_min, destination_intents.amount_out AS destination_amount_out,
  settlement_intents.amount AS settlement_amount,
  settlement_intents.asset AS settlement_asset, settlement_intents.recipient AS settlement_recipient,
  settlement_intents.domain AS settlement_domain, settlement_intents.status AS settlement_status,
  COALESCE(destination_intents.return_data, settlement_intents.return_data) AS destination_return_data,
  settlement_intents.transaction_hash AS settlement_transaction_hash, settlement_intents.timestamp AS settlement_timestamp,
  settlement_intents.block_number AS settlement_block_number, settlement_intents.gas_limit AS settlement_gas_limit,
  settlement_intents.gas_price AS settlement_gas_price, settlement_intents.tx_origin AS settlement_tx_origin,
  settlement_intents.tx_nonce AS settlement_tx_nonce, settlement_intents.auto_id AS settlement_auto_id,
  hub_intents.domain AS hub_domain, hub_intents.queue_idx AS hub_queue_idx, hub_intents.message_id AS hub_message_id,
  hub_intents.status AS hub_status, hub_intents.settlement_domain AS hub_settlement_domain,
  hub_intents.settlement_amount AS hub_settlement_amount, hub_intents.added_tx_nonce AS hub_added_tx_nonce,
  hub_intents.added_timestamp AS hub_added_timestamp, hub_intents.filled_tx_nonce AS hub_filled_tx_nonce,
  hub_intents.filled_timestamp AS hub_filled_timestamp, hub_intents.settlement_enqueued_tx_nonce AS hub_settlement_enqueued_tx_nonce,
  hub_intents.settlement_enqueued_block_number AS hub_settlement_enqueued_block_number,
  hub_intents.settlement_enqueued_timestamp AS hub_settlement_enqueued_timestamp, hub_intents.settlement_epoch AS hub_settlement_epoch,
  ...
  public.hascalldata(origin_intents.data) AS has_calldata, hub_intents.auto_id AS hub_auto_id
...
-- Recreate invoices view with order_id included
CREATE MATERIALIZED VIEW public.invoices AS SELECT * FROM (
  SELECT origin_intents.id, origin_intents.queue_idx AS origin_queue_idx, origin_intents.message_id AS origin_message_id,
  origin_intents.status AS origin_status, origin_intents.initiator AS origin_initiator, origin_intents.receiver AS origin_receiver,
  origin_intents.input_asset AS origin_input_asset, origin_intents.output_asset AS origin_output_asset, origin_intents.amount AS origin_amount,
  origin_intents.max_fee AS origin_max_fee, origin_intents.origin AS origin_origin, origin_intents.destinations AS origin_destinations,
  ...
  origin_intents.fee_adapter_initiator AS origin_fee_adapter_initiator, origin_intents.order_id AS origin_order_id,
  origin_intents.amount_out_min AS origin_amount_out_min, origin_intents.is_swap AS origin_is_swap,
  hub_invoices.id AS hub_invoice_id, hub_invoices.intent_id AS hub_invoice_intent_id, hub_invoices.amount AS hub_invoice_amount,
  hub_invoices.ticker_hash AS hub_invoice_ticker_hash, hub_invoices.owner AS hub_invoice_owner, hub_invoices.entry_epoch AS hub_invoice_entry_epoch,
  hub_invoices.enqueued_tx_nonce AS hub_invoice_enqueued_tx_nonce, hub_invoices.enqueued_timestamp AS hub_invoice_enqueued_timestamp,
  hub_invoices.auto_id AS hub_invoice_auto_id, hub_intents.status AS hub_status, hub_intents.settlement_epoch AS hub_settlement_epoch
  FROM public.hub_invoices
  LEFT JOIN public.origin_intents ON origin_intents.id = hub_invoices.intent_id
  LEFT JOIN public.hub_intents ON origin_intents.id = hub_intents.id
) AS t WITH NO DATA;

-- Recreate daily metrics views (unchanged from latest migration)
CREATE MATERIALIZED VIEW public.daily_metrics_by_chains_tokens AS
 ...
 SELECT combined.day, combined.from_chain_id, combined.from_asset_address, combined.from_asset_symbol, combined.to_chain_id,
        combined.to_asset_address, combined.to_asset_symbol, combined.netting_volume, combined.netting_avg_intent_size,
        combined.netting_protocol_revenue, combined.netting_total_intents, combined.netting_avg_time_in_hrs,
        combined.volume_settled_by_mm, combined.total_intents_by_mm, combined.discounts_by_mm, combined.avg_discounts_by_mm,
        combined.rewards_for_invoices, combined.avg_rewards_by_invoice, combined.avg_settlement_time_in_hrs_by_mm,
        combined.apy, combined.avg_discount_epoch_by_mm, combined.total_volume, combined.total_intents,
        combined.total_protocol_revenue, combined.total_rebalancing_fee
   FROM combined WITH NO DATA;
...
CREATE MATERIALIZED VIEW public.daily_metrics_by_date AS
 ...
 SELECT COALESCE(n.day, s.day) AS day, n.netting_volume, n.netting_avg_intent_size, n.netting_protocol_revenue,
        n.netting_total_intents, n.netting_avg_time_in_hrs, s.volume_settled_by_mm, s.total_intents_by_mm,
        s.discounts_by_mm, s.avg_discounts_by_mm, s.rewards_for_invoices, s.avg_rewards_by_invoice,
        s.avg_settlement_time_in_hrs_by_mm, s.apy, s.avg_discount_epoch_by_mm,
        (COALESCE(n.netting_volume, (0)::double precision) + COALESCE(s.volume_settled_by_mm, (0)::double precision)) AS total_volume,
        (COALESCE(n.netting_total_intents, (0)::bigint) + COALESCE(s.total_intents_by_mm, (0)::bigint)) AS total_intents,
        (COALESCE(n.netting_protocol_revenue, (0)::double precision) + COALESCE(s.protocol_revenue_mm, (0)::double precision)) AS total_protocol_revenue,
        ((COALESCE(n.netting_protocol_revenue, (0)::double precision) + COALESCE(s.protocol_revenue_mm, (0)::double precision)) + COALESCE(s.discounts_by_mm, (0)::double precision)) AS total_rebalancing_fee
   FROM (netted_final n FULL JOIN settled_final s ON ((n.day = s.day))) WITH NO DATA;
...
-- Recreate indexes
CREATE INDEX IF NOT EXISTS daily_metrics_by_chains_tokens_day_index ON public.daily_metrics_by_chains_tokens USING btree (day);
CREATE INDEX IF NOT EXISTS daily_metrics_by_chains_tokens_from_chain_id_index ON public.daily_metrics_by_chains_tokens USING btree (from_chain_id);
CREATE INDEX IF NOT EXISTS daily_metrics_by_chains_tokens_to_chain_id_index ON public.daily_metrics_by_chains_tokens USING btree (to_chain_id);
CREATE INDEX IF NOT EXISTS daily_metrics_by_chains_tokens_from_asset_address_index ON public.daily_metrics_by_chains_tokens USING btree (from_asset_address);
CREATE INDEX IF NOT EXISTS daily_metrics_by_chains_tokens_to_asset_address_index ON public.daily_metrics_by_chains_tokens USING btree (to_asset_address);
CREATE INDEX IF NOT EXISTS daily_metrics_by_chains_tokens_from_asset_symbol_index ON public.daily_metrics_by_chains_tokens USING btree (from_asset_symbol);
CREATE INDEX IF NOT EXISTS daily_metrics_by_chains_tokens_to_asset_symbol_index ON public.daily_metrics_by_chains_tokens USING btree (to_asset_symbol);
CREATE INDEX IF NOT EXISTS daily_metrics_by_date_day_index ON public.daily_metrics_by_date USING btree (day);
...
-- This migration is a data migration to ensure materialized views are up-to-date
-- Rolling back doesn't make sense as it would only recreate the views without order_id
-- The down migration is intentionally minimal
REFRESH MATERIALIZED VIEW public.intents;
REFRESH MATERIALIZED VIEW public.invoices;
REFRESH MATERIALIZED VIEW public.daily_metrics_by_chains_tokens;
REFRESH MATERIALIZED VIEW public.daily_metrics_by_date;
