...
-- 2. Daily Metrics by Date
-- create another materialized view with just date grouped
...
FROM cron.job
WHERE command = 'REFRESH MATERIALIZED VIEW public.daily_metrics_by_chains_tokens;';
SELECT cron.unschedule(jobid)
FROM cron.job
WHERE command = 'REFRESH MATERIALIZED VIEW public.daily_metrics_by_date;';
DROP MATERIALIZED VIEW IF EXISTS public.daily_metrics_by_chains_tokens;
DROP MATERIALIZED VIEW IF EXISTS public.daily_metrics_by_date;