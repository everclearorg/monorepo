...
DROP MATERIALIZED VIEW IF EXISTS daily_metrics_by_chains_tokens;
DROP MATERIALIZED VIEW IF EXISTS daily_metrics_by_date;
DROP MATERIALIZED VIEW IF EXISTS invoices;
DROP MATERIALIZED VIEW IF EXISTS intents;
DROP MATERIALIZED VIEW IF EXISTS invoices_with_shadow_data;
DROP MATERIALIZED VIEW IF EXISTS intents_with_shadow_data;

ALTER TABLE public.origin_intents ALTER COLUMN transaction_hash TYPE character(130);
ALTER TABLE public.settlement_intents ALTER COLUMN transaction_hash TYPE character(130);
...
netted_raw AS (
    SELECT DATE_TRUNC('day', to_timestamp(i.settlement_timestamp)) AS day,
...
-- Recreate functions with auto incrementing auto_id
