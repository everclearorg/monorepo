...
ALTER TABLE destination_intents DROP COLUMN IF EXISTS caller;
ALTER TABLE origin_intents DROP COLUMN IF EXISTS caller;
ALTER TABLE messages RENAME COLUMN "caller" to "tx_origin";
ALTER TABLE messages ADD COLUMN "gas_price" bigint NOT NULL;
ALTER TABLE messages ADD COLUMN "gas_limit" bigint NOT NULL;

-- migrate:down

ALTER TABLE messages DROP COLUMN IF EXISTS gas_price;
ALTER TABLE messages DROP COLUMN IF EXISTS gas_limit;
ALTER TABLE messages RENAME COLUMN "tx_origin" to "caller";

ALTER TABLE origin_intents ADD COLUMN "caller" character varying(66) NOT NULL;
ALTER TABLE destination_intents ADD COLUMN "caller" character varying(66) NOT NULL;

DROP VIEW IF EXISTS intents;
CREATE MATERIALIZED VIEW public.intents AS
 SELECT origin_intents.id,
    origin_intents.queue_idx AS origin_queue_idx,
    origin_intents.message_id AS origin_message_id,
