...
ALTER TABLE public.origin_intents ADD COLUMN is_swap BOOLEAN NOT NULL DEFAULT false;
CREATE INDEX idx_origin_intents_is_swap ON public.origin_intents USING btree (is_swap);

-- Drop and recreate materialized views to include the new column
...
-- Recreate intents view (adding origin_is_swap column after origin_amount_out_min)
CREATE MATERIALIZED VIEW public.intents AS SELECT * FROM (
  ...
  destination_intents.amount_out_min AS settlement_amount_out_min, settlement_intents.amount AS settlement_amount,
...
-- Recreate invoices view (adding origin_is_swap column)
...
-- Recreate metrics views (unchanged, just need to exist after intents view)
...
DROP INDEX IF EXISTS public.idx_origin_intents_is_swap;
ALTER TABLE public.origin_intents DROP COLUMN IF EXISTS is_swap;

-- Recreate views without is_swap column (omitted for brevity - would reverse the above)
