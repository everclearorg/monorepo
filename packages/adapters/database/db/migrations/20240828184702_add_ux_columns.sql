...
BEGIN
    IF origin_status = 'ADDED' AND hub_status = 'NONE' THEN
...
CREATE OR REPLACE FUNCTION hasCallData(data TEXT)
RETURNS BOOLEAN AS $$
DECLARE
    hex_part TEXT;
BEGIN
...
DROP FUNCTION IF EXISTS genStatus(TEXT, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS hasCallData(TEXT);
