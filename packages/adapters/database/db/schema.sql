...
ALTER TABLE ONLY public.queue_dispatches
    ADD CONSTRAINT queue_dispatches_pkey PRIMARY KEY (task_id);
...
CREATE UNIQUE INDEX idx_queue_dispatches_pending_unique ON public.queue_dispatches USING btree (domain, queue_type, queue_first, queue_last) WHERE ((status)::text = 'pending'::text);
