import { getRandomBytes32, RelayerApiFee, RelayerTaskStatus } from '@chimera-monorepo/utils';
 ...
 *
 * Task Errors:
 *   key: error:$taskId | value: JSON.stringify(EverclearError);
 *
 * Task Tx Hash:
...
export class TasksCache extends Cache {
  ...
  /// MARK - Task Data
  /**
   * Get recorded data field for given identifier.
  ...
  /**
   * Write data to cache and update the status of the task to be RelayerTaskStatus.ExecPending.
   *
