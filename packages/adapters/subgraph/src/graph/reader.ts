...
export class GraphReader implements ISubgraphReader {
  ...
  public static create(config: SubgraphConfig): GraphReader {
    if (GraphReader.instance) {
      return GraphReader.instance;
    }

    GraphReader.instance = new GraphReader(config);
    return GraphReader.instance;
  }
   ...
   *
   * @param domain - The domain of the subgraph you want to query.
   * @param queries - The GraphQL query string you want to send.
  ...
  public async getSpokeQueues(domain: string): Promise<Queue[]> {
    ...
    return (response?.data.queues ?? []).map((e) => parser.spokeQueue(domain, e));
  }
  ...
  public async getSettlementQueues(hubDomain: string): Promise<Queue[]> {
    ...
    return (response?.data.settlementQueues ?? []).map((e) => parser.settlementQueue(e));
  }
  ...
  public async getDepositQueues(hubDomain: string, fromBlock: number): Promise<DepositQueue[]> {
    ...
    return (response?.data.depositQueues ?? []).map((e) => parser.depositQueue(e));
  }
  ...
  public async getDepositsEnqueuedByNonce(
    ...
    return (response?.data.depositEnqueuedEvents ?? []).map((e) => parser.hubDepositFromEnqueued(e));
  }
  ...
  public async getDepositsProcessedByNonce(
    ...
    return (response?.data.depositProcessedEvents ?? []).map((e) => parser.hubDepositFromProcessed(e));
  }
