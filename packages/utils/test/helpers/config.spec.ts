...
describe('Helpers:Config', () => {
  ...
  describe('#getEverclearConfig', () => {
    ...
    it('should return undefined if config parsing fails', async () => {
      getMock.resolves({ data: { invalid: true } });
      const config = await getEverclearConfig('http://foo.com');
      expect(config).to.be.undefined;
      expect(getMock.calledOnceWith('http://foo.com')).to.be.true;
    });

    it('should return undefined and not fetch when no url provided', async () => {
      const config = await getEverclearConfig();
      expect(config).to.be.undefined;
      expect(getMock.notCalled).to.be.true;
    });
