...
export const processMessageQueue = async (type: QueueType) => {
  ...
  // Reconcile pending dispatch statuses with the relayer
  await reconcileQueueDispatches(getContext());
  ...
  const results = await Promise.allSettled(
    toDispatch.map(async (queue) => {
      // Dedup check: skip if there's already a pending dispatch for this queue slice
      const pendingDispatch = await database.getPendingQueueDispatch(
        queue.domain,
        type,
        queue.first,
        queue.last,
      ...
      if (pendingDispatch) {
        logger.info('Skipping queue dispatch - already has pending relayer task', requestContext, methodContext, {
          domain: queue.domain,
          type,
          first: queue.first,
          last: queue.last,
          existingTaskId: pendingDispatch.taskId,
          relayerType: pendingDispatch.relayerType,
          dispatchedAt: pendingDispatch.dispatchedAt,
        });
        return;
      }
      ...
      // Record dispatches for dedup tracking
      for (const taskId of taskIds) {
        try {
          await database.saveQueueDispatch(queue.domain, type, queue.first, queue.last, taskId, 'Everclear');
        } catch (err) {
          logger.warn('Failed to save queue dispatch record', requestContext, methodContext, {
            domain: queue.domain,
            type,
            taskId,
            error: err,
 ...
 */
async function reconcileQueueDispatches(context: ReturnType<typeof getContext>) {
  const {
    logger,
    adapters: { database, relayers },
  } = context;
  ...
  try {
    ...
    for (const dispatch of pendingDispatches) {
      try {
        ...
        if (newStatus) {
          await database.updateQueueDispatchStatus(dispatch.taskId, newStatus);
          updated++;
        }
      } catch (err) {
        // Don't fail the whole reconciliation if one task status check fails
