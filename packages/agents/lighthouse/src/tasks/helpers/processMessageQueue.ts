...
export const processMessageQueue = async (type: QueueType) => {
  ...
  // Use a shared in-flight promise so concurrent callers join the same reconciliation instead of
  // racing through the timestamp check and spawning duplicate runs.
  const now = Date.now();
  if (now - lastReconcileTimestamp >= RECONCILE_INTERVAL_MS) {
    if (!reconcileInFlight) {
      lastReconcileTimestamp = now;
      reconcileInFlight = reconcileQueueDispatches().finally(() => {
