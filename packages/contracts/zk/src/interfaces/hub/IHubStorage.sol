...
interface IHubStorage {
     ...
     */
    function adoptedForAssets(bytes32 _assetHash) external view returns (AssetConfig memory _config);
     ...
     */
    function discountPerEpoch(bytes32 _tickerHash) external view returns (uint24 _discountPerEpoch);
     ...
     */
    function custodiedAssets(bytes32 _assetHash) external view returns (uint256 _amount);
     ...
     */
    function deposits(uint48 _epoch, uint32 _domain, bytes32 _tickerHash)
        external
        view
        returns (uint256 _first, uint256 _last, uint256 _firstDepositWithPurchasePower);
     ...
     */
    function depositsAvailableInEpoch(uint48 _epoch, uint32 _domain, bytes32 _tickerHash)
        external
        view
     ...
     */
    function contexts(bytes32 _intentId) external view returns (IntentContext memory _intentContext);
     ...
     */
    function invoices(bytes32 _tickerHash)
        external
        view
        returns (bytes32 _head, bytes32 _tail, uint256 _nonce, uint256 _length);
     ...
     */
    function roles(address _account) external view returns (Role _role);
     ...
     */
    function settlements(uint32 _domain) external view returns (uint256 _first, uint256 _last);
     ...
     */
    function domainGasLimit(uint32 _domain) external view returns (uint256 _blockGasLimit);
     ...
     */
    function tokenConfigs(bytes32 _tickerHash)
        external
        view
     ...
     */
    function tokenFees(bytes32 _tickerHash) external view returns (Fee[] memory _fees);
