...
library TypedMemView {
  ...
  error TypeAssertionFailed(uint256 actual, uint256 expected);
  error Overrun(uint256 loc, uint256 len, uint256 index, uint256 slice);
  error IndexMoreThan32Bytes();
  error NullPointer();
  ...
  function assertValid(
    ...
    if (isNotValid(memView)) revert ValidityAssertionFailed();
    return memView;
  }
  ...
  function typeOf(
    bytes29 memView
  ) internal pure returns (uint40 _type) {
    assembly {
      // solhint-disable-previous-line no-inline-assembly
      // 216 == 256 - 40
  ...
  function loc(
    bytes29 memView
  ) internal pure returns (uint96 _loc) {
  ...
  function len(
    bytes29 memView
  ) internal pure returns (uint96 _len) {
    uint256 _mask = LOW_12_MASK; // assembly can't use globals
    assembly {
      // solhint-disable-previous-line no-inline-assembly
  ...
  function index(bytes29 memView, uint256 _index, uint8 _bytes) internal pure returns (bytes32 result) {
    ...
    if (_index + _bytes > len(memView)) {
      ...
      revert Overrun(loc(memView), len(memView), _index, uint256(_bytes));
    }
    if (_bytes > 32) revert IndexMoreThan32Bytes();
