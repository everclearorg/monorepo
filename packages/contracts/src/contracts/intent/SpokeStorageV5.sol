...
abstract contract SpokeStorageV5 is ISpokeStorageV5 {
  ...
  bytes32 public constant FILL_INTENT_TYPEHASH = keccak256(
    'function fillIntent(bytes32 _domain, address _sender, address _receiver, Intent calldata _intent, uint256 _amountOut, uint32[] memory _destinations)'
  );

  /// @inheritdoc ISpokeStorageV5
  bytes32 public constant BATCH_FILL_INTENT_TYPEHASH = keccak256(
    'function batchFillIntent(bytes32 _domain, address _sender, address[] calldata _receivers, Intent[] calldata _intents, uint256[] _amountOut, uint32[][] memory _destinations)'
  );
