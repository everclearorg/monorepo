  ...
  IMessageReceiver
{
  ...
  function batchFillIntent(
    BatchFill calldata _batchFill
  ) external whenNotPaused returns (FillMessage[] memory _fillMessages) {
    uint256 length = _batchFill.intents.length;
    if (
      length != _batchFill.amountOut.length || length != _batchFill.receivers.length
        || length != _batchFill.destinations.length
    ) {
    ...
    bytes memory _data = abi.encode(
      ...
      _domain,
      msg.sender,
      _batchFill.receivers,
      _batchFill.intents,
      _batchFill.amountOut,
      _batchFill.destinations
    );
    _verifySignature(fillSigner, _data, _batchFill.signature);

    _fillMessages = new FillMessage[](length);
    for (uint256 i; i < length; i++) {
      _fillMessages[i] = _fillIntent(
        _batchFill.intents[i],
        msg.sender,
        _batchFill.receivers[i],
        _batchFill.amountOut[i],
        _batchFill.destinations[i],
        false
      );
    }
  }
  ...
  function batchFillIntentWithPull(
    ...
    _fillMessages = new FillMessage[](_batchFill.intents.length);
    for (uint256 i; i < _batchFill.intents.length; i++) {
      _fillMessages[i] = _fillIntent(
        _batchFill.intents[i],
        msg.sender,
        _batchFill.receivers[i],
  ...
  function fillIntent(
    ...
    bytes32 _domain = keccak256(abi.encode(block.chainid, address(this)));
    bytes memory _data =
      abi.encode(FILL_INTENT_TYPEHASH, _domain, msg.sender, _receiver, _intent, _amountOut, _destinations);
  ...
  function fillIntentForSolver(
    ...
    bytes memory _fillData = abi.encode(
