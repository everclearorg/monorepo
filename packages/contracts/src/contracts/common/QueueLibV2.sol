...
library QueueLibV2 {
  ...
  struct FillQueue {
    uint256 first;
    uint256 last;
    mapping(uint256 _position => IEverclearV2.FillMessage _fillMessage) queue;
  }
   ...
   */
  function enqueueFill(FillQueue storage _queue, IEverclearV2.FillMessage memory _fillMessage) internal {
    _queue.last += 1;
