 ...
 */
contract SettlerV2 is SettlerLogicV2, ISettlerV2, IEverclearV2 {
  ...
  function _processInvoice(
    ...
    _settled = true;
    if (_domainResult.strategy == IEverclearV2.Strategy.XERC20) {
      ...
      HubQueueLibV2.DepositQueue storage _depositQueue = deposits[_epoch][_domainResult.selectedDomain][_tickerHash];
  ...
  function _processInvoices(
    ...
    InvoiceListLibV2.InvoiceList storage _invoiceList = invoices[_tickerHash];
    ...
    if (_lastClosedEpoch > _lastClosedEpochProcessed || _lastClosedEpoch == 0) {
      ...
      for (_i = _lastClosedEpochProcessed; _i <= _lastClosedEpoch; _i++) {
        ...
        for (uint256 _j; _j < _assetDomains.length; _j++) {
          ...
          HubQueueLibV2.DepositQueue storage _depositQueue = deposits[_i][_domain][_tickerHash];
  ...
  function _findDestinationWithStrategiesForInvoice(
    ...
    if (_tokenConfigs[_tickerHash].prioritizedStrategy == IEverclearV2.Strategy.XERC20) {
      ...
      if (_domainResult.selectedDomain == 0) {
        ...
        _domainResult.strategy = IEverclearV2.Strategy.XERC20;
