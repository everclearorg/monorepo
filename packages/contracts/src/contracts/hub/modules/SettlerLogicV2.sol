...
abstract contract SettlerLogicV2 is HubStorageV2 {
  ...
  function _createSettlement(
    ...
    _contexts[_intentId].status = IEverclearV2.IntentStatus.SETTLED;
    bytes32 _assetHash = _tokenConfigs[_tickerHash].assetHashes[_destination];
    IEverclearV2.Strategy _strategy = _adoptedForAssets[_assetHash].strategy;
    ...
    IEverclearV2.Settlement memory _settlement = IEverclearV2.Settlement({
      intentId: _intentId,
      amount: _amount,
      asset: _asset,
  ...
  function _findDestinationXerc20Strategy(
    ...
    for (uint256 _i; _i < _destinations.length; _i++) {
      ...
      bytes32 _assetHash = _tokenConfigs[_tickerHash].assetHashes[_destination];
      if (_adoptedForAssets[_assetHash].strategy == IEverclearV2.Strategy.XERC20) {
