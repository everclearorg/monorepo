...
library HubQueueLibV2 {
  ...
  struct DepositQueue {
    ...
    mapping(uint256 _position => IHubStorageV2.Deposit _deposit) queue;
  }
  ...
  struct SettlementQueue {
    uint256 first;
    uint256 last;
    mapping(uint256 _position => IEverclearV2.Settlement _settlement) queue;
  }
  ...
  ) internal returns (IHubStorageV2.Deposit memory _deposit) {
  ...
  ) internal returns (IEverclearV2.Settlement memory _settlement) {
    // non-empty queue
    if (_queue.last < _queue.first || _queue.first == 0) revert Queue_EmptyQueue();
