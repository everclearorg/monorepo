...
export async function assetDashboard(): Promise<void> {
  ...
  let script = `assets/${environment.toLowerCase()}/${choice.contractName}.s.sol:${choice.contractName}Dashboard`;

  // add environment and domain details
  script += ` --rpc-url ${domain!.rpc} --chain ${domain!.id}`;

  // choose account to broadcast from
  script += ` --sig "run(address)" ${address}`;
