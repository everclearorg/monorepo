...
mod tests {
    ...
    fn test_fill_signer_separate_from_fee_signer() {
        ...
        assert_ne!(
            ...
            "Fee signer and fill signer must be different to enforce PoLP"
        );
    }
    ...
    fn test_update_fill_signer_independent_of_fee_signer() {
        ...
        assert_ne!(
            fee_signer_unchanged, fill_signer_updated,
            "Updating fill_signer should not affect fee_signer"
        );
    }
    ...
    fn test_fill_signer_update_preserves_fee_signer() {
        ...
        assert_ne!(old_fill_signer, new_fill_signer);
        
        let fee_signer_before_update = fee_signer;
        let fee_signer_after_update = fee_signer;
        
        assert_eq!(
            fee_signer_before_update, fee_signer_after_update,
            "Fee signer should remain unchanged when fill signer is updated"
        );
    }
    ...
    fn test_separation_enforces_principle_of_least_privilege() {
        ...
        let fee_signer_can_sign_fees = true;
        let fee_signer_can_sign_fills = fee_signer == fill_signer;
        let fill_signer_can_sign_fills = true;
        let fill_signer_can_sign_fees = fill_signer == fee_signer;
        
        assert!(
            fee_signer_can_sign_fees && !fee_signer_can_sign_fills,
            "Fee signer should only be able to sign fees, not fills"
        );
        
        assert!(
            fill_signer_can_sign_fills && !fill_signer_can_sign_fees,
            "Fill signer should only be able to sign fills, not fees"
        );
    }
    ...
    fn test_multiple_fill_signer_updates() {
        ...
        assert_ne!(fill_signer_1, fill_signer_3);
        
        let can_update_multiple_times = fill_signer_1 != fill_signer_2 && fill_signer_2 != fill_signer_3;
        
        assert!(
            can_update_multiple_times,
            "Fill signer should be updatable multiple times independently"
        );
    }
}
