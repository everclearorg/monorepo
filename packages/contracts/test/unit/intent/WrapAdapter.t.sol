...
contract BaseTest is TestExtended {
  ...
  uint256 immutable RELAYER_PK = 1;
  uint256 immutable RELAYER2_PK = 2;
  address immutable RELAYER = vm.addr(RELAYER_PK);
  address immutable RELAYER2 = vm.addr(RELAYER2_PK);
  ...
  function mockExecuteIntentCalldata(IEverclear.Intent memory _intent) internal {
    vm.mockCall(address(SPOKE), abi.encodeWithSelector(IEverclearSpoke.executeIntentCalldata.selector), '');
  ...
  function mockWETHWithdraw(uint256 _amount) internal {
    vm.mockCall(address(WETH), abi.encodeWithSelector(IWETH.withdraw.selector, _amount), '');
  ...
  function mockWETHDeposit(uint256 _amount) internal {
    vm.mockCall(address(WETH), _amount, abi.encodeWithSelector(IWETH.deposit.selector), '');
  ...
  function mockWETHApprove(address _spender, uint256 _amount) internal {
    vm.mockCall(address(WETH), abi.encodeWithSelector(IWETH.approve.selector, _spender, _amount), abi.encode(true));
    vm.expectCall(address(WETH), abi.encodeWithSelector(IWETH.approve.selector, _spender, _amount));
  }

  function mockStatus(bytes32 _intentId, IEverclear.IntentStatus _status) internal {
    vm.mockCall(address(SPOKE), abi.encodeWithSelector(ISpokeStorage.status.selector), abi.encode(_status));
  ...
  function _addRelayerAndGenerateSignature(
    bytes32 _intentId,
    address _relayer,
    uint256 _relayerPk
  ) internal returns (bytes memory _sig) {
    // Configuring update relayer input
    address[] memory _relayerArray = new address[](1);
    _relayerArray[0] = _relayer;
    bool[] memory _statusArray = new bool[](1);
    _statusArray[0] = true;

    // Updating the relayer array
    vm.prank(wrapAdapter.owner());
    wrapAdapter.updateRelayer(_relayerArray, _statusArray);
    assertEq(wrapAdapter.approvedRelayer(_relayer), true);

    // Generating signature from the relayer
    return _generateSignature(_intentId, _relayerPk);
  }

  function _generateSignature(bytes32 _intentId, uint256 _relayerPk) internal returns (bytes memory _sig) {
    (uint8 v, bytes32 r, bytes32 s) = vm.sign(_relayerPk, _intentId);
...
contract Unit_WrapAdapter_BatchUnwrapIntent is BaseTest {
  ...
  function test_batchUnwrapIntent_MixedBatchSuccess(
    IEverclear.Intent[2] memory _intent,
    uint256[2] memory _amountOut,
    address[2] memory _unwrapReceiver
  ) public {
    ...
    bytes memory _adapterCallbackCalldata = abi.encodeWithSelector(
      WrapAdapter.adapterCallback.selector,
      _unwrapReceiver[0]
    );
    _intent[0].data = abi.encode(address(wrapAdapter), _adapterCallbackCalldata);

    // Deploy and deal XERC20 for Intent[0]
    (bytes32 _nativeToken, bytes32 _xerc20token) = deployAndDealXERC20Token(
      address(wrapAdapter),
      address(wrapAdapter).toBytes32(),
      _amountOut[0]
    );
    ...
    _intentIds[0] = keccak256(abi.encode(_intent[0]));
    _intentIds[1] = keccak256(abi.encode(_intent[1]));
    bytes[] memory _signature = new bytes[](2);
    _signature[0] = _addRelayerAndGenerateSignature(_intentIds[0], RELAYER, RELAYER_PK);
    _signature[1] = _generateSignature(_intentIds[1], RELAYER_PK);

    // Executing the batch transaction
    vm.startPrank(wrapAdapter.owner());
    wrapAdapter.batchUnwrapIntent(_intentArray, _amounts, _signature);
    vm.stopPrank();

    // Confirming the active storage value is zeroed after the run
    assertEq(wrapAdapter.active(), 0);
  }

  function test_Revert_batchUnwrapIntent_InvalidRelayer() public {
    IEverclear.Intent[] memory _intents = new IEverclear.Intent[](2);
    uint256[] memory _amountsOut = new uint256[](2);
    bytes[] memory _signature = new bytes[](2);
    _signature[0] = _generateSignature(keccak256(abi.encode(_intents[0])), RELAYER_PK);
    _signature[1] = _generateSignature(keccak256(abi.encode(_intents[1])), RELAYER_PK);

    vm.expectRevert(abi.encodeWithSelector(IWrapAdapter.Invalid_Relayer.selector, RELAYER));
    wrapAdapter.batchUnwrapIntent(_intents, _amountsOut, _signature);
  }

  function test_Revert_batchUnwrapIntent_InvalidArrayLength_IntentAndAmountComparison() external {
    ...
    wrapAdapter.batchUnwrapIntent(_intent, _amountOut, _signature);
    vm.stopPrank();
  }

  function test_Revert_batchUnwrapIntent_InvalidArrayLength_IntentAndSignatureComparison() external {
    IEverclear.Intent[] memory _intent = new IEverclear.Intent[](2);
    uint256[] memory _amountOut = new uint256[](2);
    bytes[] memory _signature = new bytes[](3);
...
contract Unit_WrapAdapter_UnwrapIntent is BaseTest {
  ...
  function test_unwrapIntent_XERC20Success(
    IEverclear.Intent memory _intent,
    uint256 _amountOut,
    address _unwrapReceiver
  ) public {
    ...
    // Generating signature
    bytes32 _intentId = keccak256(abi.encode(_intent));
    bytes memory _signature = _addRelayerAndGenerateSignature(_intentId, RELAYER, RELAYER_PK);

    // Unwrapping the asset
    wrapAdapter.unwrapIntent(_intent, _amountOut, _signature);
    vm.stopPrank();
  ...
  function test_Revert_unwrapIntent_InvalidRelayer() public {
    IEverclear.Intent memory _intent;
    uint256 _amountOut;
    bytes32 _intentId = keccak256(abi.encode(_intent));
    bytes memory _signature = _generateSignature(_intentId, RELAYER_PK);
  ...
  function test_Revert_unwrapIntent_InvalidCallback(
    IEverclear.Intent memory _intent,
    uint256 _amountOut,
  ...
  function test_Revert_unwrapIntent_WETH_TransferETHFailure(
    IEverclear.Intent memory _intent,
    uint256 _amountOut
  ) public {
    vm.assume(_intent.amount > 0);
...
contract Unit_WrapAdapter_UnwrapInvalidIntent is BaseTest {
  ...
  function test_unwrapInvalidIntent_Success_XERC20(
    IEverclear.Intent memory _intent,
    uint256 _amountOut,
    address _unwrapReceiver
  ) public {
    ...
    // Unwrapping the asset
    vm.startPrank(wrapAdapter.owner());
    wrapAdapter.unwrapInvalidIntent(_intent, _amountOut, _unwrapReceiver, _signature);
    vm.stopPrank();
  ...
  function test_Revert_unwrapInvalidIntent_InvalidRelayer(
    IEverclear.Intent memory _intent,
    uint256 _amount,
    address _receiver
  ) public {
    bytes memory _signature = _generateSignature(keccak256(abi.encode(_intent)), RELAYER_PK);

    vm.expectRevert(abi.encodeWithSelector(IWrapAdapter.Invalid_Relayer.selector, RELAYER));
    wrapAdapter.unwrapInvalidIntent(_intent, _amount, _receiver, _signature);
  }

  function test_Revert_unwrapInvalidIntent_InvalidAlreadyProcessed(
    IEverclear.Intent memory _intent,
    uint256 _amountOut,
    address _unwrapReceiver
  ) public {
  ...
  function test_Revert_unwrapInvalidIntent_InvalidReceiver(
    IEverclear.Intent memory _intent,
    uint256 _amount,
    address _receiver
  ) public {
  ...
  function test_Revert_unwrapInvalidIntent_InvalidOutputAmount(
    IEverclear.Intent memory _intent,
    uint256 _amountOut,
    address _receiver
  ) public {
    ...
    wrapAdapter.unwrapInvalidIntent(_intent, _amountOut, _receiver, _signature);
    vm.stopPrank();
  }

  function test_Revert_unwrapInvalidIntent_InvalidManualProcessing(
    IEverclear.Intent memory _intent,
    uint256 _amountOut,
    address _receiver
  ) public {
  ...
  function test_Revert_unwrapInvalidIntent_InvalidStatus(
    IEverclear.Intent memory _intent,
    uint256 _amountOut,
    address _receiver
  ) public {
...
contract Unit_WrapAdapter_AdapterCallback is BaseTest {
  ...
  function test_adapterCallback_InvalidActivity() public {
    bytes memory _message;

    vm.startPrank(address(wrapAdapter.everclearSpoke()));
    vm.expectRevert(abi.encodeWithSelector(IWrapAdapter.Invalid_Activity.selector));
    wrapAdapter.adapterCallback(_message);
    vm.stopPrank();
  }

  //     // TODO: Need to mock storage
...
contract Unit_WrapAdapter_WrapAndSendIntent is BaseTest {
   ...
   */
  function test_Revert_wrapAndSendIntent_InvalidInputAmount(
    IEverclear.Intent memory _intent,
    IERC20 _assetToWrap,
    address _caller
  ) public {
...
contract Unit_ValidateIntentCalldata is BaseTest {
  ...
  function test_validateIntentCalldata_InvalidCallReceiver(
    IEverclear.Intent memory _intent,
    address _unwrapReceiver
  ) public {
    // Configuring the correct calldata
  ...
  function test_validateIntentCalldata_InvalidFuncSelector(
    IEverclear.Intent memory _intent,
    address _unwrapReceiver
  ) public {
    vm.assume(_unwrapReceiver != address(0));
...
contract Unit_WrapAdapter_DecodeArbitraryData is BaseTest {
  function test_decodeArbitraryData_Success(address _unwrapReceiver) public {
    ...
    bytes memory _data = abi.encode(_callReceiver, _adapterCallbackCalldata);
    (address _decodedCallReceiver, address _decodedUnwrapReceiver, bytes4 _decodedFuncSelector) = wrapAdapter
      .decodeArbitraryData(_data);
  ...
  function test_decodeArbitraryData_160BytesButCalldataNot36() public {
    ...
    (address callReceiver, address unwrapReceiver, bytes4 funcSelector) = wrapAdapter.decodeArbitraryData(data);
